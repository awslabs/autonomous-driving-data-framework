...
def main(metadata: dict):
  ...
  payload = {"bucket": metadata['BucketName'], "key": metadata['Key'], "record_id": metadata['RecordID'], "scene_id": metadata['SceneID']}
  logging.info(f"Payload: {json.dumps(payload)}")
  response = client.invoke(
    FunctionName=str(metadata['FunctionName']), InvocationType="RequestResponse", LogType="Tail", Payload=json.dumps(payload)
  )
...
if __name__ == "__main__":
  ...
  parser.add_argument(CONFIG_FILE_OPT,
    dest="config_file",
    required=False,
