...
def _add_branch(tree: dict, branch: list, value: str) -> dict:
    ...
    """
    key = branch[0]
    tree[key] = value if len(branch) == 1 else _add_branch(tree[key] if key in tree else {}, branch[1:], value)
    return tree
...
def _needs_custom_replication(data: dict, image_name: str, type_of_value: str) -> bool:
    ...
    Returns:
        bool: Boolean value
    """
    if "replication" in data and image_name in data["replication"] and type_of_value in data["replication"][image_name]:
        return True
...
def parse_value(workload: dict, values: dict, image_name: str, image_data: dict, value_name: str) -> str:
    ...
    if "subchart" in image_data:
        ...
        value = _get_dictionary_value_by_dot_separated_key(workload[image_data["location"]], image_data["path"])
