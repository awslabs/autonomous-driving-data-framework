            ...
            if repo["repositoryName"].startswith(prefix):
                ...
                try:
                    ...
                    if ex.response["Error"]["Code"] == "RepositoryNotFoundException":
                        print(
                            "Repository: {} is not found, no action needed".format(
                                repo["repositoryName"]
                            )
                        )
                        continue
        ...
        for entry in page["Parameters"]:
            ...
            try:
                ...
                if ex.response["Error"]["Code"] == "ParameterNotFound":
                    print(
                        "SSM PARAMETER: {} is not found, no action needed".format(
                            entry["Name"]
                        )
                    )
                    continue
...
cleanup_ecr_repos(prefix="addf")

# Cleanups SSM Parameters based on a specific path pattern
cleanup_ssm_params(path="/addf/eks/chart")
cleanup_ssm_params(path="/addf/eks/ami")
