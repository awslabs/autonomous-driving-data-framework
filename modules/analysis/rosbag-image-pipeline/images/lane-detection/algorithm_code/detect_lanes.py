...
def get_yolov5_prediction(model, image, input_size=1280, confidence = 0.25,  iou = 0.45):
    ...
    output['category'] = [
        category_list[int(x)] for x in output['category_index']
    ]
...
if __name__ == "__main__":
    ...
    for image in images_list:
        ...
        output[image.replace("/opt/ml/processing/input/", "")] = output_json

        with open(f"{output_data_path}{image.replace('.png', '.json')}", 'w') as file:
            dump_json(output_json, file, indent=2, separators=(',', ': '), sort_keys=False)

        output_pandas.to_csv(f"{output_data_path}{image.replace('.png', '.json')}", encoding="utf-8")
