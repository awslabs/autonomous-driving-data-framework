...
def is_object_in_lane(obj, lane_points):
    ...
    TODO
        - Maybe try only looking at the bottom left and right corners of the image
        - adapt new data model to this format:
        old object looks like this = {
            "x": 1,
            "y": 1,
            "width": 1,
            "height": 1,
    ...
    lanes = []
    for obj_corner in [
        (obj_corner_x_min, obj_corner_y_min),  # Bottom left
        (obj_corner_x_max, obj_corner_y_min),  # Bottom right
...
def obj_in_lane_detection(row):
    """"
    TODO
        - sensor names are hardcoded, not good
        - they should be an input of the obje

    - look at new output from new ML models

    """
    object_detection_col_name = "rgb_right_detections_only_clean"
    lane_detection_col_name = "post_process_lane_points_rgb_front_right_clean"

    if row.get(object_detection_col_name) and row.get(lane_detection_col_name):
        objects_in_lane = []
...
def detect_scenes(synchronized_data):
    """
    TODO Expects Object detection topic and lane detection topic, on a unified timeline,
        if they come from the same camera sensor stream this should be a given.

    @param synchronized_data:
    @return:
    """
