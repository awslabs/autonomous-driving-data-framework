...
class EmrContainersBaseSensor(BaseSensorOperator):
    """
    Contains general sensor behavior for EMR Containers.

    Subclasses should implement following methods:
        - ``get_emr_containers_response()``
        - ``state_from_response()``
    ...
    def get_hook(self):
        """Get EmrContainersHook"""
        if not self.hook:
            self.hook = EmrContainersHook(aws_conn_id=self.aws_conn_id)
        return self.hook

    def poke(self, context):
        response = self.get_emr_containers_response()
        ...
        self.log.info('Job ID - current state is  %s', state)

        if state in self.target_states:
            return True

        if state in self.failed_states:
            final_message = 'EMR Containers job failed'
    ...
    def get_emr_containers_response(self) -> Dict[str, Any]:
        ...
        :rtype: dict[str, Any]
        """
        raise NotImplementedError('Please implement get_emr_containers_response() in subclass')
